...
const isMeetingPossible = (startOfDay, endOfDay, startOfMeeting, timeForMeeting) => {
  const timeLeft = timeToMinutes(endOfDay) - timeToMinutes(startOfMeeting);
  if (timeToMinutes(startOfMeeting) >= timeToMinutes(startOfDay)) {
    return timeForMeeting <= timeLeft;
  } return false;
